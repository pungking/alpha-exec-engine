...
### 2026-06-18 - Live Readiness Scorecard and Observation Stop Rules

- Scope: `alpha-exec-engine` / report-only live-readiness classification; no broker, state, or multi-submit mutation.
- Added `live-readiness-scorecard`:
  - writes `state/live-readiness-scorecard.json` / `.md`.
  - separates paper submit capability, open/fill/expired/canceled lifecycle consistency, ledger/idempotency state, protective order/guard metadata, scheduler/fresh-hash status, broker mutation safety, Stage6 entry quality, and open-order reprice guard.
  - emits final verdict as one of `BLOCKED`, `PAPER_PILOT`, or `MICRO_LIVE_REVIEW_READY`.
  - keeps `MICRO_LIVE_REVIEW_READY` unreachable while terminal ledger reconciliation, guard metadata, broker-child protection, or current mutation-safety blockers remain.
- Entry-order lifecycle is classified from all dynamic order-state evidence:
  - submitted/open waiting is not treated as completed success.
  - filled quantity `0` remains `OPEN_WAITING` while broker-open evidence is valid.
  - reprice policy `WAIT_POLICY_NOT_REPRICE_CANDIDATE` remains waiting, not a mutation candidate.
  - duplicate-open evidence must remain clean before any later action.
- `ops-health-report=fail` is split inside the scorecard instead of treated as one opaque failure:
  - Stage6/entry blockers,
  - protective order/guard metadata blockers,
  - ledger terminal reconciliation blockers,
  - scheduler/fresh hash blockers,
  - broker mutation safety blockers.
- Observation stop rules are now first-class output:
  - fresh hash with same no-actionable/no-event state gets one safe run only, then observation stops.
  - repeated zero-executable routes to Stage6 producer tuning, not passive waiting.
  - unchanged open orders are not rechecked until fill/expire/cancel or approved replace event.
  - broker or state mutation still requires exact scoped approval.

Done-When:

- Safe sidecar artifacts include `live-readiness-scorecard.json` / `.md`.
- Scorecard shows current paper submit evidence separately from live-readiness blockers.
- Dynamic open orders are scored as waiting/incomplete until filled, terminal, or an approved replace event occurs.
- `brokerMutationAttempted=false` and `brokerMutationSubmitted=false` remain required for any scorecard generated by normal safe runs.
- Repeated hash/no-event observation has explicit stop conditions and routes to producer tuning when appropriate.

### 2026-06-18 - Entry-Order Terminalization Proposal for Expired Pullback Orders

- Scope: `alpha-exec-engine` / report-only ledger terminalization proposal; no state write and no broker mutation.
- Extended `ledger-terminalization-proposal` beyond position/fill-state rows so entry orders that closed unfilled can be proposed for terminal ledger/idempotency cleanup.
- The proposal now consumes `fillability-report.json` and `order-state-consistency-report.json` in addition to broker fill-state evidence.
- Entry-order terminal candidates require:
  - `fillability.status=TERMINAL_UNFILLED`,
  - terminal state such as `expired`, `canceled`, or `rejected`,
  - matching ledger/idempotency rows,
  - order-state terminal reconciliation evidence when available.
- Expired pullback orders are classified as `PROPOSE_ENTRY_ORDER_LEDGER_IDEMPOTENCY_MARK_TERMINAL_UNFILLED` with patch preview only.
- Same Stage6/hash re-entry remains blocked until a fresh Stage6 or explicit retry approval exists.

Done-When:

- `ledger-terminalization-proposal.summary.entryTerminalUnfilledReady > 0` when an entry order expires unfilled and ledger/idempotency still say submitted.
- The proposal includes patch previews but keeps `stateMutationAllowed=false` and `stateMutationAttempted=false`.
- `live-readiness-scorecard` keeps live readiness `BLOCKED` while terminal reconciliation remains unapplied.
- Any actual ledger/idempotency write remains a separate state-only migration with backup, diff, audit, and post-verify.

### 2026-07-16 - Symbol-Agnostic Entry Lifecycle Scorecard v2

- Scope: `alpha-exec-engine` report-only readiness evaluation; no broker or state mutation.
- `live-readiness-scorecard.json` schema advances from `1.0.0` to `2.0.0`.
- Breaking migration:
  - removes the ticker-specific `mliLifecycle` canonical field,
  - adds `entryOrderLifecycle` with deterministic rows derived from `order-state-consistency-report.json`,
  - uses fillability, ledger, idempotency, and reprice reports only to supplement missing canonical evidence,
  - does not keep a compatibility alias because repository consumer inspection found no runtime consumer of `mliLifecycle`.
- Lifecycle classification is symbol-agnostic: `FILLED_COMPLETE`, `OPEN_WAITING`, `CONSISTENT_TERMINAL`, `TERMINAL_RECONCILIATION_REQUIRED`, `SUBMITTED_EVIDENCE_ONLY`, or `NO_LIFECYCLE_EVIDENCE`.
- Planned-only candidates without broker lifecycle evidence are excluded; absence of an active order is not itself a readiness blocker.
- Filled-position protection gaps remain exclusively in `protective_order_guard_metadata` and are not counted again as lifecycle blockers.
- `PAPER_PILOT` and `MICRO_LIVE_REVIEW_READY` now depend only on aggregate dynamic lifecycle evidence and mutation-safety domains.

Done-When:

- Runtime code contains no proof ticker lifecycle lookup.
- Rename-invariant synthetic fixtures preserve lifecycle aggregates and final verdict.
- Terminal reconciliation, duplicate-open, and unknown lifecycle rows block readiness; valid open orders remain waiting.
