...
- MLI lifecycle is explicitly classified:
  - submitted/open waiting is not treated as completed success.
  - filled quantity `0` remains `WAITING_OPEN_NOT_FILLED`.
...
- MLI-like open orders are scored as waiting/incomplete until filled, terminal, or approved replace event occurs.
...
- MLI-like expired pullback orders are classified as `PROPOSE_ENTRY_ORDER_LEDGER_IDEMPOTENCY_MARK_TERMINAL_UNFILLED` with patch preview only.
