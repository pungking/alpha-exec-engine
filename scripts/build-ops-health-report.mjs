...
const FILES = {
  ...
  stage6FillabilityMismatchAudit: `${STATE_DIR}/stage6-fillability-mismatch-audit.json`,
...
const buildMarkdown = (report) => {
  ...
  lines.push(
    `- ownership_guard_gap: \`overall=${report.metrics.ownershipGuardGapOverall ?? "N/A"} rows=${report.metrics.ownershipGuardGapRows ?? "N/A"} protected=${report.metrics.ownershipGuardGapProtected ?? "N/A"} externalAndGuardMissing=${report.metrics.ownershipGuardGapExternalAndGuardMissing ?? "N/A"} ownershipReview=${report.metrics.ownershipGuardGapOwnershipReview ?? "N/A"} guardMissing=${report.metrics.ownershipGuardGapGuardMissing ?? "N/A"} repairEligible=${report.metrics.ownershipGuardGapRepairEligible ?? "N/A"} multiEligible=${report.metrics.ownershipGuardGapMultiEligible ?? "N/A"} attempted=${report.metrics.ownershipGuardGapAttempted ?? "N/A"} submitted=${report.metrics.ownershipGuardGapSubmitted ?? "N/A"}\``
  );
  lines.push(
    `- ownership_recovery_decision: \`overall=${report.metrics.ownershipRecoveryDecisionOverall ?? "N/A"} rows=${report.metrics.ownershipRecoveryDecisionRows ?? "N/A"} stateReady=${report.metrics.ownershipRecoveryStateReady ?? "N/A"} externalAdoptionReview=${report.metrics.ownershipRecoveryExternalAdoptionReview ?? "N/A"} alreadyProtected=${report.metrics.ownershipRecoveryAlreadyProtected ?? "N/A"} doNotAutoRecover=${report.metrics.ownershipRecoveryDoNotAutoRecover ?? "N/A"} repairEligibleAfterRecovery=${report.metrics.ownershipRecoveryRepairEligibleAfterRecovery ?? "N/A"} attempted=${report.metrics.ownershipRecoveryAttempted ?? "N/A"} submitted=${report.metrics.ownershipRecoverySubmitted ?? "N/A"} stateAttempted=${report.metrics.ownershipRecoveryStateAttempted ?? "N/A"} stateApplied=${report.metrics.ownershipRecoveryStateApplied ?? "N/A"}\``
  );
  lines.push(
...
const main = () => {
  ...
  const stage6FillabilityMismatchAudit = readJson(FILES.stage6FillabilityMismatchAudit);
  ...
  if (fillability && entryRepricePolicyDecision && !stage6FillabilityMismatchAudit) {
    addCheck(
      checks,
      "warn",
      "stage6_fillability_mismatch_audit_missing",
      "state/stage6-fillability-mismatch-audit.json not found; Stage6 executable vs sidecar fillability mismatch was not audited"
    );
  }
  ...
  const ownershipGuardGapOverall = short(positionOwnershipGuardGapAudit?.overall || "", 64) || null;
  const ownershipGuardGapRows = toNum(positionOwnershipGuardGapAudit?.summary?.rows);
  const ownershipGuardGapProtected = toNum(positionOwnershipGuardGapAudit?.summary?.alreadyProtectedNoAction);
  const ownershipGuardGapExternalAndGuardMissing = toNum(positionOwnershipGuardGapAudit?.summary?.externalPositionAndGuardMissing);
  const ownershipGuardGapOwnershipReview = toNum(positionOwnershipGuardGapAudit?.summary?.externalPositionOwnershipReview);
  const ownershipGuardGapGuardMissing = toNum(positionOwnershipGuardGapAudit?.summary?.guardMetadataMissingSourceGap);
  const ownershipGuardGapManualApproval = toNum(positionOwnershipGuardGapAudit?.summary?.manualApprovalCandidates);
  const ownershipGuardGapRepairEligible = toNum(positionOwnershipGuardGapAudit?.summary?.repairEligible);
  const ownershipGuardGapMultiEligible = toNum(positionOwnershipGuardGapAudit?.summary?.multiPlannerEligible);
  const ownershipGuardGapAttempted =
    positionOwnershipGuardGapAudit?.executionPolicy?.brokerMutationAttempted === true ||
    positionOwnershipGuardGapAudit?.summary?.brokerMutationAttempted === true;
  const ownershipGuardGapSubmitted =
    positionOwnershipGuardGapAudit?.executionPolicy?.brokerMutationSubmitted === true ||
    positionOwnershipGuardGapAudit?.summary?.brokerMutationSubmitted === true;
  const ownershipGuardGapBrokerMutationAllowed =
    positionOwnershipGuardGapAudit?.executionPolicy?.brokerMutationAllowed === true;
  const ownershipRecoveryDecisionOverall = short(positionOwnershipRecoveryDecision?.overall || "", 64) || null;
  const ownershipRecoveryDecisionRows = toNum(positionOwnershipRecoveryDecision?.summary?.rows);
  const ownershipRecoveryStateReady = toNum(positionOwnershipRecoveryDecision?.summary?.stateRecoveryReviewReady);
  const ownershipRecoveryExternalAdoptionReview = toNum(
    positionOwnershipRecoveryDecision?.summary?.manualExternalAdoptionReview
  );
  const ownershipRecoveryAlreadyProtected = toNum(positionOwnershipRecoveryDecision?.summary?.alreadyProtectedNoRecovery);
  const ownershipRecoveryDoNotAutoRecover = toNum(positionOwnershipRecoveryDecision?.summary?.doNotAutoRecover);
  const ownershipRecoveryRepairEligibleAfterRecovery = toNum(
    positionOwnershipRecoveryDecision?.summary?.repairEligibleAfterRecovery
  );
  const ownershipRecoveryAttempted =
  ...
  const ownershipRecoverySubmitted =
  ...
  const ownershipRecoveryStateAttempted =
  ...
  const ownershipRecoveryStateApplied =
    positionOwnershipRecoveryDecision?.executionPolicy?.stateMutationApplied === true ||
    positionOwnershipRecoveryDecision?.summary?.stateMutationApplied === true;
  const ownershipRecoveryBrokerMutationAllowed =
    positionOwnershipRecoveryDecision?.executionPolicy?.brokerMutationAllowed === true;
  const ownershipRecoveryStateMutationAllowed =
    positionOwnershipRecoveryDecision?.executionPolicy?.stateMutationAllowed === true;
  const ownershipRecoveryGateOverall = short(positionOwnershipRecoveryApprovalGate?.overall || "", 64) || null;
  const ownershipRecoveryGateRows = toNum(positionOwnershipRecoveryApprovalGate?.summary?.rows);
  const ownershipRecoveryGateStateReady = toNum(positionOwnershipRecoveryApprovalGate?.summary?.stateRecoveryReviewReady);
  const ownershipRecoveryGateExternalAdoptionReview = toNum(
    positionOwnershipRecoveryApprovalGate?.summary?.manualExternalAdoptionReview
  );
  const ownershipRecoveryGateDoNotAutoRecover = toNum(positionOwnershipRecoveryApprovalGate?.summary?.doNotAutoRecover);
  const ownershipRecoveryGateApprovalProvided =
    positionOwnershipRecoveryApprovalGate?.summary?.approvalProvided === true ||
    positionOwnershipRecoveryApprovalGate?.executionPolicy?.approvalProvided === true;
  const ownershipRecoveryGateAttempted =
  ...
  const ownershipRecoveryGateSubmitted =
  ...
  const ownershipRecoveryGateStateAttempted =
  ...
  const ownershipRecoveryGateStateApplied =
  ...
  const ownershipRecoveryGateBrokerMutationAllowed =
    positionOwnershipRecoveryApprovalGate?.executionPolicy?.brokerMutationAllowed === true;
  const ownershipRecoveryGateStateMutationAllowed =
  ...
  const stage6FillabilityMismatchOverall = short(stage6FillabilityMismatchAudit?.overall || "", 64) || null;
  const stage6FillabilityMismatchRows = toNum(stage6FillabilityMismatchAudit?.summary?.mismatchRows);
  const stage6FillabilityMismatchReviewReady = toNum(
    stage6FillabilityMismatchAudit?.summary?.stage6QualityReviewReady
  );
  const stage6FillabilityMismatchWaitPullback = toNum(
    stage6FillabilityMismatchAudit?.summary?.waitPullbackRows
  );
  const stage6FillabilityMismatchEntryReady = toNum(
    stage6FillabilityMismatchAudit?.summary?.entryPolicyReviewReady
  );
  const stage6FillabilityMismatchAttempted =
    stage6FillabilityMismatchAudit?.executionPolicy?.brokerMutationAttempted === true ||
    stage6FillabilityMismatchAudit?.summary?.brokerMutationAttempted === true;
  const stage6FillabilityMismatchSubmitted =
    stage6FillabilityMismatchAudit?.executionPolicy?.brokerMutationSubmitted === true ||
    stage6FillabilityMismatchAudit?.summary?.brokerMutationSubmitted === true;
  const stage6FillabilityMismatchBrokerMutationAllowed =
    stage6FillabilityMismatchAudit?.executionPolicy?.brokerMutationAllowed === true;
  ...
  if (
    ownershipGuardGapBrokerMutationAllowed ||
    ownershipGuardGapAttempted ||
    ownershipGuardGapSubmitted
  ) {
    addCheck(
      checks,
      "fail",
      "position_ownership_guard_gap_unsafe",
      `ownership/guard gap audit must remain non-mutating; brokerMutationAllowed=${ownershipGuardGapBrokerMutationAllowed} attempted=${ownershipGuardGapAttempted} submitted=${ownershipGuardGapSubmitted}`
    );
  }

  if (
    (ownershipGuardGapExternalAndGuardMissing ?? 0) > 0 ||
    (ownershipGuardGapOwnershipReview ?? 0) > 0 ||
    (ownershipGuardGapGuardMissing ?? 0) > 0
  ) {
    addCheck(
      checks,
      "warn",
      "position_ownership_guard_gap_review_required",
      `ownership/guard gap audit found externalAndGuardMissing=${ownershipGuardGapExternalAndGuardMissing ?? "N/A"} ownershipReview=${ownershipGuardGapOwnershipReview ?? "N/A"} guardMissing=${ownershipGuardGapGuardMissing ?? "N/A"}; keep repair blocked until ownership and fresh guard source evidence exist`
    );
  } else if (ownershipGuardGapOverall) {
    addCheck(
      checks,
      "pass",
      "position_ownership_guard_gap_clear",
      `ownership/guard gap audit clear or monitor-only; protected=${ownershipGuardGapProtected ?? "N/A"} repairEligible=${ownershipGuardGapRepairEligible ?? "N/A"}`
    );
  }

  if (
    ownershipRecoveryBrokerMutationAllowed ||
    ownershipRecoveryStateMutationAllowed ||
    ownershipRecoveryAttempted ||
    ownershipRecoverySubmitted ||
    ownershipRecoveryStateAttempted ||
    ownershipRecoveryStateApplied
  ) {
    addCheck(
      checks,
      "fail",
      "position_ownership_recovery_decision_unsafe",
      `ownership recovery decision must remain report-only; brokerAllowed=${ownershipRecoveryBrokerMutationAllowed} stateAllowed=${ownershipRecoveryStateMutationAllowed} attempted=${ownershipRecoveryAttempted} submitted=${ownershipRecoverySubmitted} stateAttempted=${ownershipRecoveryStateAttempted} stateApplied=${ownershipRecoveryStateApplied}`
    );
  }

  if ((ownershipRecoveryExternalAdoptionReview ?? 0) > 0 || (ownershipRecoveryDoNotAutoRecover ?? 0) > 0) {
    addCheck(
      checks,
      "warn",
      "position_ownership_recovery_external_adoption_required",
      `ownership recovery decision found external/manual rows requiring explicit adoption evidence; externalAdoptionReview=${ownershipRecoveryExternalAdoptionReview ?? "N/A"} doNotAutoRecover=${ownershipRecoveryDoNotAutoRecover ?? "N/A"} stateReady=${ownershipRecoveryStateReady ?? "N/A"}`
    );
  } else if ((ownershipRecoveryStateReady ?? 0) > 0) {
    addCheck(
      checks,
      "warn",
      "position_ownership_recovery_state_review_ready",
      `ownership recovery has ${ownershipRecoveryStateReady ?? "N/A"} state-only review-ready row(s); applying it requires a separate approved backup/diff/audit/post-verify migration`
    );
  } else if (ownershipRecoveryDecisionOverall) {
    addCheck(
      checks,
      "pass",
      "position_ownership_recovery_decision_clear",
      `ownership recovery decision is monitor-only; alreadyProtected=${ownershipRecoveryAlreadyProtected ?? "N/A"}`
    );
  }

  if (
    ownershipRecoveryGateBrokerMutationAllowed ||
    ownershipRecoveryGateStateMutationAllowed ||
    ownershipRecoveryGateMultiSubmitAllowed ||
    ownershipRecoveryGateAttempted ||
    ownershipRecoveryGateSubmitted ||
    ...
    addCheck(
      checks,
      "fail",
      "position_ownership_recovery_gate_unsafe",
      `ownership recovery approval gate must remain report-only; brokerAllowed=${ownershipRecoveryGateBrokerMutationAllowed} stateAllowed=${ownershipRecoveryGateStateMutationAllowed} multiSubmit=${ownershipRecoveryGateMultiSubmitAllowed} attempted=${ownershipRecoveryGateAttempted} submitted=${ownershipRecoveryGateSubmitted} stateAttempted=${ownershipRecoveryGateStateAttempted} stateApplied=${ownershipRecoveryGateStateApplied}`
    );
  }

  if (ownershipRecoveryGateOverall === "blocked_external_adoption_evidence_required") {
    addCheck(
      checks,
      "warn",
      "position_ownership_recovery_gate_external_evidence_required",
      `ownership recovery approval gate blocked external/manual adoption; externalAdoptionReview=${ownershipRecoveryGateExternalAdoptionReview ?? "N/A"} doNotAutoRecover=${ownershipRecoveryGateDoNotAutoRecover ?? "N/A"}`
    );
  } else if (ownershipRecoveryGateOverall === "manual_state_approval_required") {
    addCheck(
      checks,
      "warn",
      "position_ownership_recovery_gate_state_approval_required",
      `ownership recovery has ${ownershipRecoveryGateStateReady ?? "N/A"} state-ready row(s) but exact state approval is absent`
    );
  } else if (ownershipRecoveryGateOverall === "state_recovery_review_authorized_report_only") {
    addCheck(
      checks,
      "warn",
      "position_ownership_recovery_gate_review_authorized",
      "state recovery review is authorized report-only; a separate migration task must still provide backup/diff/audit/post-verify before applying state"
    );
  } else if (ownershipRecoveryGateOverall) {
    addCheck(
      checks,
      "pass",
      "position_ownership_recovery_gate_clear",
      `ownership recovery approval gate is monitor-only; approvalProvided=${ownershipRecoveryGateApprovalProvided}`
    );
  }
  ...
  if (
    stage6FillabilityMismatchBrokerMutationAllowed ||
    stage6FillabilityMismatchAttempted ||
    stage6FillabilityMismatchSubmitted
  ) {
    addCheck(
      checks,
      "fail",
      "stage6_fillability_mismatch_audit_unsafe",
      `Stage6/fillability mismatch audit must remain report-only; brokerMutationAllowed=${stage6FillabilityMismatchBrokerMutationAllowed} attempted=${stage6FillabilityMismatchAttempted} submitted=${stage6FillabilityMismatchSubmitted}`
    );
  }

  if (stage6FillabilityMismatchReviewReady != null && stage6FillabilityMismatchReviewReady > 0) {
    addCheck(
      checks,
      "warn",
      "stage6_fillability_mismatch_review_ready",
      `${stage6FillabilityMismatchReviewReady} Stage6 executable row(s) were blocked by sidecar fillability; route to Stage6 entry timing/fillability calibration before submit or replace`
    );
  }
  ...
  const metrics = {
      ...
      stage6FillabilityMismatchOverall,
      stage6FillabilityMismatchRows,
      stage6FillabilityMismatchReviewReady,
      stage6FillabilityMismatchWaitPullback,
      stage6FillabilityMismatchEntryReady,
      stage6FillabilityMismatchAttempted,
      stage6FillabilityMismatchSubmitted,
      orderStateOverall,
      orderStateFailures,
      ...
      limitedMultiOcoRepairAttempted,
      limitedMultiOcoRepairSubmitted,
      ownershipGuardGapOverall,
      ownershipGuardGapRows,
      ownershipGuardGapProtected,
      ownershipGuardGapExternalAndGuardMissing,
      ownershipGuardGapOwnershipReview,
      ownershipGuardGapGuardMissing,
      ownershipGuardGapManualApproval,
      ownershipGuardGapRepairEligible,
      ownershipGuardGapMultiEligible,
      ownershipGuardGapAttempted,
      ownershipGuardGapSubmitted,
      ownershipRecoveryDecisionOverall,
      ownershipRecoveryDecisionRows,
      ownershipRecoveryStateReady,
      ownershipRecoveryExternalAdoptionReview,
      ownershipRecoveryAlreadyProtected,
      ownershipRecoveryDoNotAutoRecover,
      ownershipRecoveryRepairEligibleAfterRecovery,
      ownershipRecoveryAttempted,
      ownershipRecoverySubmitted,
      ownershipRecoveryStateAttempted,
      ownershipRecoveryStateApplied,
      ownershipRecoveryGateOverall,
      ownershipRecoveryGateRows,
      ownershipRecoveryGateStateReady,
      ownershipRecoveryGateExternalAdoptionReview,
      ownershipRecoveryGateDoNotAutoRecover,
      ownershipRecoveryGateApprovalProvided,
      ownershipRecoveryGateAttempted,
      ownershipRecoveryGateSubmitted,
      ownershipRecoveryGateStateAttempted,
      ownershipRecoveryGateStateApplied,
